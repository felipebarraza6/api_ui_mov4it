...
from api.move4it.models import ApprovedCourse
...
def validate_corfo(sender, instance, **kwargs):
    ...
    response_token = requests.request(
        "POST", urlToken, headers=headersGetToken, data=payloadFormx, verify=False)
    ...
    if (response_token.status_code == 200):
        response = requests.request(
            "POST", urlValidate, headers=headersValidate, data=dataJson, verify=False)
        print(response.text)
